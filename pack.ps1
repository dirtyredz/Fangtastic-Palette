<#
    Builds a release archive laid out the way Nexus and Vortex expect:
        ...
        BepInEx/plugins/FangtasticPalette/FangtasticPalette.dll

    Deliberately not the dev deploy path (plugins/MoonlightPeaksMods/FangtasticPalette), which
...
$repoRoot = $modRoot
$project  = Join-Path $modRoot 'src\FangtasticPalette.csproj'
...
Write-Host "Packing Fangtastic Palette $version"
...
$dll = Join-Path $modRoot 'src\bin\Release\netstandard2.1\FangtasticPalette.dll'
if (-not (Test-Path $dll)) { throw "Built DLL not found at $dll" }

$staging = Join-Path $env:TEMP "FangtasticPalette-pack-$([guid]::NewGuid().ToString('N'))"
$target  = Join-Path $staging 'BepInEx\plugins\FangtasticPalette'
...
$archive = Join-Path $dist "FangtasticPalette-$version.zip"
